...
[ExecuteInEditMode]
[CustomEditor(typeof(Weather_Cloudy))]
public class WeatherCloudy_Editor : Editor
{
    ...
    {
        Weather_Cloudy cl = target as Weather_Cloudy;
        ...
        if (bShowTips == true)
            EditorGUILayout.HelpBox(("If you want a special sound to be played during this weather effect turn this on."), MessageType.Info, true);
        ...
        {
            if (bShowTips == true)
                EditorGUILayout.HelpBox(("Sound (GameObject) - The gameobject from where the sound will be played! \n\nSound (AudioClip) - The audiofile with the sound you want to play \n\nThe fade timer is how fast you want the sound to fade in and out when it changes weather effect."), MessageType.Info, true);

            GUILayout.BeginHorizontal();
            GUILayout.Label("Sound (Gameobject): ");
            cl.GetSet_gSoundEffect = EditorGUILayout.ObjectField("", cl.GetSet_gSoundEffect, typeof(GameObject), true, GUILayout.MaxWidth(iMaxWidth)) as GameObject;
            GUILayout.EndHorizontal();
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label("Sound time to fade in and out (in sec.): ");
