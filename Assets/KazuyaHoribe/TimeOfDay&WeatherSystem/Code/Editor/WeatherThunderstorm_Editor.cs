...
[ExecuteInEditMode]
[CustomEditor(typeof(Weather_Thunderstorm))]
public class WeatherThunderstorm_Editor : Editor
{
    ...
    {
        Weather_Thunderstorm cl = target as Weather_Thunderstorm;
        ...
        if (bShowTips == true)
            EditorGUILayout.HelpBox(("Add your rain particle prefab to this"), MessageType.Info, true);

        GUILayout.BeginHorizontal();
        GUILayout.Label("Rain particles (Gameobject): ");
        cl.GetSet_gPartRain = EditorGUILayout.ObjectField("", cl.GetSet_gPartRain, typeof(GameObject), true, GUILayout.MaxWidth(iMaxWidth)) as GameObject;
        GUILayout.EndHorizontal();
        ...
        if (cl.GetSet_bUsingSound == true)
        {
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label("Rain sound (AudioClip): ");
            ...
            // THUNDER
            GUILayout.BeginHorizontal();
            GUILayout.Label("Thunder sound (AudioClip): ");
            cl.GetSet_adThunderSound = EditorGUILayout.ObjectField("", cl.GetSet_adThunderSound, typeof(AudioClip), true, GUILayout.MaxWidth(iMaxWidth)) as AudioClip;
            GUILayout.EndHorizontal();

            GUILayout.BeginHorizontal();
            GUILayout.Label("Lightning sound volume: ");
            cl.GetSet_fLightningVolume = EditorGUILayout.FloatField("", cl.GetSet_fLightningVolume, GUILayout.MaxWidth(iMedWidth));
            GUILayout.EndHorizontal();
        }

        /* ----- THUNDER SETTINGS ----- */
        EditorGUILayout.HelpBox(("Thunder settings"), MessageType.None, true);
        if (bShowTips == true)
            EditorGUILayout.HelpBox(("Here you set all the settings for lightnings. It picks a random number between the lowest and highest value when the next lightning should strike."), MessageType.Info, true);

        GUILayout.BeginHorizontal();
        GUILayout.Label("Next thunder in sec. (min): ");
        cl.GetSet_RandomThunderTimeMin = EditorGUILayout.FloatField("", cl.GetSet_RandomThunderTimeMin, GUILayout.MaxWidth(iMedWidth));
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("Next thunder in sec. (max): ");
        cl.GetSet_RandomThunderTimeMax = EditorGUILayout.FloatField("", cl.GetSet_RandomThunderTimeMax, GUILayout.MaxWidth(iMedWidth));
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("Lightning (Light) intensity (max): ");
        cl.GetSet_fLightningIntensity = EditorGUILayout.FloatField("", cl.GetSet_fLightningIntensity, GUILayout.MaxWidth(iMedWidth));
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("Lightning (Light) range: ");
        cl.GetSet_fLightningRange = EditorGUILayout.FloatField("", cl.GetSet_fLightningRange, GUILayout.MaxWidth(iMedWidth));
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("How long should the light from the lightning be seen: ");
        cl.GetSet_fTimeBeforeLightningLightTurnsOff = EditorGUILayout.FloatField("", cl.GetSet_fTimeBeforeLightningLightTurnsOff, GUILayout.MaxWidth(iMedWidth));
        GUILayout.EndHorizontal();

        /* ----- THUNDER LOCATION SETTINGS ----- */
        if (bShowTips == true)
            EditorGUILayout.HelpBox(("This is the settings for where the thunder randomly will spawn. You set a minimun and maximum distance from the Weathermaster. \n\n*These values can be negative"), MessageType.Info, true);

        GUILayout.BeginHorizontal();
        GUILayout.Label("Lightning spawn location min. distance (from Weathermaster): ");
        cl.GetSet_fLightningRangeFromWeatherMasterMin = EditorGUILayout.FloatField("", cl.GetSet_fLightningRangeFromWeatherMasterMin, GUILayout.MaxWidth(iMedWidth));
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("Lightning spawn location max. distance (from Weathermaster): ");
        cl.GetSet_fLightningRangeFromWeatherMasterMax = EditorGUILayout.FloatField("", cl.GetSet_fLightningRangeFromWeatherMasterMax, GUILayout.MaxWidth(iMedWidth));
        GUILayout.EndHorizontal();

        GUILayout.BeginHorizontal();
        GUILayout.Label("Lightning spawn height location (from Weathermaster): ");
        cl.GetSet_fLightningHeight = EditorGUILayout.FloatField("", cl.GetSet_fLightningHeight, GUILayout.MaxWidth(iMedWidth));
        GUILayout.EndHorizontal();
